...
class AopCertClient
{
	 ...
	 * @return $this
	 */
	public function setEnv($env = 'sandbox'): self
	{
		if ($env === 'sandbox') {
	 ...
	 * @return self
	 */
	public function setRsaPrivateKey(string $rsaPrivateKey): self
	{
	 ...
	 * @return self
	 */
	public function setAlipayrsaPublicKey(string $alipayRsaPublicKey): self
	{
	...
	/**
	 * @param string $appCertSn
	 * @return self
	 */
	public function setAppCertSN(string $appCertSn): self
	{
	...
	/**
	 * @param string $alipayRootCertSn
	 * @return self
	 */
	public function setAlipayRootCertSN(string $alipayRootCertSn): self
	{
	...
	/**
	 * @param mixed $isCheckAlipayPublicCert
	 * @return self
	 */
	public function setIsCheckAlipayPublicCert($isCheckAlipayPublicCert): self
	{
		$this->isCheckAlipayPublicCert = $isCheckAlipayPublicCert;
	 ...
	 * @return string
	 */
	public function getCertSN(string $certPath)
	{
		$cert = file_get_contents($certPath);
		return $this->getCertSNByContent($cert);
	}
	 ...
	 * @return string
	 */
	public function getCertSNByContent($cert)
	{
		$ssl = openssl_x509_parse($cert);
		return md5(AopCertEncrypt::array2string(array_reverse($ssl['issuer'])) . $ssl['serialNumber']);
	}
	 ...
	 * @return string|null
	 */
	public function getRootCertSN(string $certPath)
	{
		$cert = file_get_contents($certPath);
		return $this->getRootCertSNByContent($cert);
	}
	 ...
	 * @return string|null
	 */
	public function getRootCertSNByContent(string $cert)
	{
	 ...
	 * @param bool   $isCheckSign
	 * @param bool   $isDecrypt
	 * @param string $signType
	 * @return mixed|string
	 */
	public function checkSignAndDecrypt(array $params, string $rsaPublicKeyFilePath, bool $isCheckSign, bool $isDecrypt, $signType = 'RSA')
	{
		$bizContent = $params['biz_content'];
		if ($isCheckSign) {
			if (!$this->rsaCheckV2($params, $rsaPublicKeyFilePath, $signType)) {
				echo "<br/>checkSign failure<br/>";
				exit;
			}
		}
		if ($isDecrypt) {
			return $this->rsaDecrypt($bizContent);
		}
	 ...
	 * @return string
	 */
	public function encryptAndSign(string $bizContent, string $rsaPublicKeyFilePath, string $charset, bool $isEncrypt, bool $isSign, $signType = 'RSA')
	{
		// 加密，并签名
		if ($isEncrypt && $isSign) {
			$encrypted = $this->rsaEncrypt($bizContent, $rsaPublicKeyFilePath, $charset);
	 ...
	 * @return string
	 */
	public function rsaEncrypt(string $data, string $rsaPublicKeyFilePath, $charset)
	{
		...
		foreach ($blocks as $n => $block) {
			if (!openssl_public_encrypt($block, $chrtext, $res, 1)) {
	 ...
	 * @return string
	 */
	public function rsaDecrypt(string $data)
	{
		...
		foreach ($decodes as $n => $decode) {
			if (!openssl_private_decrypt($decode, $dcyCont, $res, 1)) {
				echo "<br/>" . openssl_error_string() . "<br/>";
			}
			$strnull .= $dcyCont;
		}
	...
	public function splitCN($cont, $n, $subnum, $charset)
	{
		//$len=strlen($cont)/3;
		$arrr = [];
		for ($i = $n, $iMax = strlen($cont); $i < $iMax; $i += $subnum) {
	 ...
	 * @param string $privatekey  商户私钥，根据 $keyfromfile 来判断是读取字符串还是读取文件，false:填写私钥字符串去回车和空格 true:填写私钥文件路径
				...
				if (!$checkResult) {
					...
					if (!empty($resultCode) && $resultCode == 10000 && !empty($certContent)) {
						...
						if (!empty($this->alipayRootCertContent) && $this->isCheckAlipayPublicCert) {
							$certCheck = AopCertEncrypt::isTrusted($cert, $this->alipayRootCertContent);
						}
						if ($certCheck) {
							$pkey                     = openssl_pkey_get_public($cert);
							$keyData                  = openssl_pkey_get_details($pkey);
